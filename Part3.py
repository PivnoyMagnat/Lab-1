...
for date in reversed(dates):
    ...
    rates.append(rate)
dates_formatted = [(today - timedelta(days=i)).strftime("%d.%m") for i in reversed(range(days))]
plt.figure(figsize=(10, 6))
plt.plot(dates_formatted, rates, marker="o", linestyle="-", color="blue", label=f"Курс {currency}")
